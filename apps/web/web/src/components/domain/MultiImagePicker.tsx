...
/** Galería de hasta 10 fotos para el formulario de producto (antes una sola). */
export function MultiImagePicker({ imagenes, onChange }: { imagenes: string[]; onChange: (imagenes: string[]) => void }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
        {imagenes.map((uri, i) => (
          <div key={i} style={{ position: "relative", width: 78, height: 78, borderRadius: "var(--radius-md)", border: "1px solid var(--border)", overflow: "hidden" }}>
            ...
            {i === 0 && (
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, textAlign: "center", padding: "2px 0", background: "var(--cyan)" }}>
      ...
      {imagenes.length > 0 && <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 6 }}>La primera foto es la portada. Haz clic en la X para quitar una.</p>}
    </div>
  );
}
