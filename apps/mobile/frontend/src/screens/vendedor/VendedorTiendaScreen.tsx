...
import { CaretLeftIcon, CaretRightIcon, CheckIcon, CrosshairIcon, MagnifyingGlassIcon, MapPinLineIcon, StorefrontIcon } from "phosphor-react-native";
              ...
              <>
                ...
                <View style={[styles.resumen, { backgroundColor: tokens.surface1, borderColor: tokens.border }]}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                    <View style={[styles.resumenLogo, { backgroundColor: tokens.surface2 }]}>
                      {logo ? <Image source={{ uri: logo }} style={StyleSheet.absoluteFill} /> : <StorefrontIcon size={16} color={tokens.textMuted} />}
                    </View>
                    <View style={{ flex: 1, minWidth: 0 }}>
                      <Text numberOfLines={1} style={{ fontSize: 14, fontFamily: "SpaceGrotesk_600SemiBold", color: tokens.textPrimary }}>{nombre || "Tu tienda"}</Text>
                      <Text numberOfLines={1} style={{ fontSize: 11.5, color: tokens.textMuted }}>{municipio}{direccion ? ` · ${direccion}` : ""}</Text>
                    </View>
                  </View>
                  {categorias.length > 0 && (
                    <Text style={{ fontSize: 11.5, color: tokens.textSecondary, marginTop: 10 }}>
...
const styles = StyleSheet.create({
  ...
  resumen: { borderRadius: 14, borderWidth: 1, padding: 14 },
  resumenLogo: { width: 36, height: 36, borderRadius: 10, alignItems: "center", justifyContent: "center", overflow: "hidden" },
